...
func ExampleSelect() {
	...
	// Sample02
	query, args, err = querybuilder.Select("table1").Columns("c1,c2,c3").
		Build()
	if err != nil {
		log.Printf("err: %s", err)
	}
	...
	// Sample03
	query, args, err = querybuilder.Select("table1").Columns("c1,c2,c3").Where("c1=true").Where("c2=?", 10).Build()
	if err != nil {
		log.Printf("err: %s", err)
	}
