...
func TestSelectQuery_Build(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:           "test7",
			query:          s.Table("table1").Columns("field1").Where("id > ?", 120).Order("timestamp"),
		...
		{
			name:           "test8",
			query:          s.Table("table1").Columns("field1").Where("id > ?", 120).Order("timestamp").Group("field1,field2"),
		...
		{
			name:           "test9",
			query:          s.Table("table1").Columns("field1").Where("id > ?", 120).Order("timestamp").Group("field1,field2").Limit(1000).Offset(0),
